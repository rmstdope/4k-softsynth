...
class WaveformDisplay:
        ...
        if MATPLOTLIB_AVAILABLE:
            try:
                ...
                if hasattr(self.main_editor, 'logger'):
                    self.main_editor.logger.warning("Failed to create waveform plot: %s", e)
                self._create_fallback_display(parent_frame, 
                                           f"Waveform display unavailable: {e}")
        else:
            self._create_fallback_display(parent_frame, 
    ...
    def _create_fallback_display(self, parent_frame, message):
        ...
        fallback_frame = ttk.Frame(parent_frame, relief="sunken", borderwidth=2)
        fallback_frame.pack(fill=tk.BOTH, expand=True)
        
        label = ttk.Label(fallback_frame, text=message,
    ...
    def show_audio_graph(self, audio_data):
        ...
        try:
            ...
            if hasattr(self.main_editor, 'status_panel'):
                self.main_editor.status_panel.log_output("✗ Cannot show graph: matplotlib not available")
        except (RuntimeError, ValueError) as e:
