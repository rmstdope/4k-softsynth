    ...
    mCurrentMode(MODE_INSTRUMENT)
{
    ...
    mEditMode[MODE_TRACK] = 
        new TrackEdit(static_cast<PatternEdit*>(mEditMode[MODE_PATTERN]), 
                      NUM_INSTRUMENTS, NUM_PATTERNS);
}
...
void
AudioEditor::AudioCallback(Uint8 *stream, 
                           int len)
{
