...
import synth_engine  # pylint: disable=import-error,c-extension-no-member
...
class TestSynthWrapperConstants:
    ...
    def test_get_constants_values_are_numeric(self, wrapper):
        ...
        for key, value in constants.items():
            assert isinstance(value, (int, float)), f"Constant '{key}' value is not numeric: {value}"
...
def create_test_audio_samples(length: int = 1000, frequency: float = 440.0, sample_rate: int = 44100):
